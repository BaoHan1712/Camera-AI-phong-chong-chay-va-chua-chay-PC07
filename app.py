...
def generate_frames(camera_id):
    ...
    while True:
        ...
        # Cập nhật trạng thái cảnh báo với thông tin camera
        fire_alerts[camera_id] = {
            'has_fire': fire_detected,
            'has_smoke': smoke_detected,
            'camera_name': f'Camera {int(camera_id[-1]) + 1}'
        }
...
# Kiểm tra cảnh báo từ camera
@app.route('/check_alerts/<camera_id>')
def check_alerts(camera_id):
    alert_info = fire_alerts.get(camera_id, {
        'has_fire': False,
        'has_smoke': False,
        'camera_name': f'Camera {int(camera_id[-1]) + 1}'
    })
